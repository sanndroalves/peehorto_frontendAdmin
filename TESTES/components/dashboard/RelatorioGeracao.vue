    ...
    const { data: real } = await useFetch(`http://localhost:8000/relatoriogeracao`);
        ...
        const somarIndividualReal = async (anoId) => {
            ...
            const updateItemsBasedOnYear = async (newYear) => {
                ...
                const { data: dadosSalvar } = await useFetch(`http://localhost:8000/salvar?ano=${newYear}`);
                const idSalvar = ref("")
                ...
                if (dadosSalvar.value && dadosSalvar.value[0]) {
                    ...
                    await useFetch(`http://localhost:8000/salvar/${idSalvar.value}`, {
                            method: 'DELETE',
                            key: 'deleteDados'
                        });
                }

                    await useFetch(`http://localhost:8000/salvar/`, {
                        method: 'POST',
                        body: 
                            {
